...
export function frontDoorPassword(word) {
  return word[0].toUpperCase() + word.slice(1, 20).toLowerCase();
}
...
export function backDoorResponse(line) {
  const lineWithTrim = line.trim();
  const lenghtOfLine = lineWithTrim.length;
  return lineWithTrim.slice(lenghtOfLine - 1);
}
